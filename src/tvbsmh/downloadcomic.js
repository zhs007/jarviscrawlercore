...
/**
 * tvbsmhDownloadComic - download comic
 ...
 */
async function tvbsmhDownloadComic(
    isdebug,
    comicid,
    bookid,
    excludebookid,
  ...
  try {
    ...
    log.error('tvbsmhDownloadComic mkdirSync error ', err);
  }
  ...
  while (true) {
    manhuaret = await tvbsmhManhua(browser, comicid, timeout);
    if (manhuaret.error) {
      log.error('tvbsmhDownloadComic error ', manhuaret.error);
  ...
  for (let i = 0; i < manhuaret.ret.books.length; ++i) {
    ...
    if (curbookret) {
      log.error('tvbsmhDownloadComic:downloadBook error ', curbookret);
    }
  }
...
async function downloadBook(browser, comicid, bookid, rootpath, timeout, dl) {
  ...
  let lastpi;
  while (true) {
    ret = await tvbsmhBook(browser, comicid, bookid, 1, timeout);
    if (ret.error) {
    ...
    if (lastpi[0] == 1) {
      if (!isValidURL(ret.ret.pages[0].url)) {
        log.error('downloadBook.tvbsmhBook isValidURL', {
  ...
  for (let i = 0; i < lastpi.length; ++i) {
    ...
    while (true) {
      ...
      if (curret.error) {
        log.error('downloadBook.tvbsmhBook error', curret.error);
      ...
      if (isValidURL(curret.ret.pages[0].url)) {
        ...
        // );
        fs.writeFileSync(
            path.join(rootpath, lastpi[i] + '.jpg'),
