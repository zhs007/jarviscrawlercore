...
async function taobaoItemMobile(browser, itemid, device, cfgdevice, timeout) {
  ...
  if (detailobj.obj && detailobj.obj.data && detailobj.obj.data.item) {
    ...
    if (curitem.commentCount) {
      ...
      if (cc.error) {
        log.warn('taobaoItemMobile.string2int commentCount', cc.error);
      } else {
        ret.reviews = cc.num;
      }
    }
    ...
    if (curitem.favcount) {
      ...
      if (fc.error) {
        log.warn('taobaoItemMobile.string2int favcount', fc.error);
      } else {
        ret.favCount = fc.num;
      }
    }
  ...
  if (detailobj.obj && detailobj.obj.data && detailobj.obj.data.seller) {
    ...
    if (seller.creditLevel) {
      ...
      if (cl.error) {
        log.warn('taobaoItemMobile.string2int seller.creditLevel', cl.error);
      } else {
    ...
    if (seller.allItemCount) {
      ...
      if (aic.error) {
        log.warn('taobaoItemMobile.string2int seller.allItemCount', aic.error);
      } else {
    ...
    if (seller.newItemCount) {
      ...
      if (nic.error) {
        log.warn('taobaoItemMobile.string2int seller.newItemCount', nic.error);
      } else {
    ...
    if (seller.goodRatePercentage) {
      ...
      if (grp.error) {
        log.warn(
            'taobaoItemMobile.percentage2float seller.goodRatePercentage',
            grp.error,
        );
      } else {
