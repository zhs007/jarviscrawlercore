...
async function getQuestion(page) {
  ...
  const ret = await page
      ...
      .catch((err) => {
        log.error('zhihu.answer:getQuestion.evaluate', err);
...
async function exportArticle(page) {
  ...
  const dom = await page.$eval('.Card.AnswerCard', (element) => {
  ...
  const ret = await page
      ...
      .catch((err) => {
        log.error('zhihu.answer:exportArticle.evaluate', err);

        errret = err;
      });

  await page.waitForFunction('window.waitimgs == 0').catch((err) => {
    log.error('zhihu.answer.formatArticle', err);
  });
