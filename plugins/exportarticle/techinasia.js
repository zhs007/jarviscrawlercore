...
async function exportArticle(page) {
  await page.waitForSelector('.infinite-scroll').catch((err) => {
    log.error('techinasia.article.exportArticle', err);
  });

  const dom = await page.$eval('.site-main-content', (element) => {
  ...
  const ret = await page
          ...
          if (articlenode) {
            for (let i = 0; i < articlenode.children.length; ++i) {
              ...
              if (articlenode.children[i].tagName == 'DIV') {
                const curimgs = articlenode.children[i].getElementsByTagName(
                    'img'
                );
                if (curimgs.length > 0) {
      ...
      .catch((err) => {
        log.error('techinasia.article:exportArticle.evaluate', err);
